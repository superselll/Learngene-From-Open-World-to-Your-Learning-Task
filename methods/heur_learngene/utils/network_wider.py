...
class Netwider(nn.Module):
    ...
    def __init__(self, num_conv, num_classes=80):
        ...
        image_length = 84
        image_width = 84
        self.classifier = make_classifier(self.output_channel * (int(image_length / (2 ** self.num_pooling))) * (
            int(image_width / (2 ** self.num_pooling))), \
                                          self.num_class)
